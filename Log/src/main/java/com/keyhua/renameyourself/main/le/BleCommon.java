...
import com.keyhua.renameyourself.main.protocol.HwtxCommandQueryBTFW;
import com.keyhua.renameyourself.main.protocol.HwtxCommandQueryLOGO;
import com.keyhua.renameyourself.main.protocol.HwtxCommandQueryMCUFW;
import com.keyhua.renameyourself.main.protocol.HwtxCommandQueryMode;
import com.keyhua.renameyourself.main.protocol.HwtxCommandQuerySNNUM;
...
import com.keyhua.renameyourself.main.protocol.HwtxCommandTXB;
...
//                }
     ...
     * 查询LOGO--------------------------------------《领队工具》----------------------------------------------------------------
     */
    private HwtxCommandQueryLOGO commandqueryLOGO = null;
    private byte[] tagQueryLOGO = null;

    public void queryLOGO() {
        if (mConnected) {
            System.out
                    .println("查询log:\n");
            if (commandqueryLOGO == null) {
                commandqueryLOGO = new HwtxCommandQueryLOGO();
            }
            if (tagQueryLOGO == null) {
                tagQueryLOGO = commandqueryLOGO.getCommandTagRaw();
            }

            sendData(commandqueryLOGO.getBluetoothPropertyUuidSend(),
                    commandqueryLOGO.getBluetoothPropertyUuidRead(),
                    commandqueryLOGO.toBytes());
     ...
     * 获取MCU FW版本号
     */
    private HwtxCommandQueryMCUFW commandqueryMCUFW = null;
    private byte[] tagQueryMCUFW = null;

    public void queryMCUFW() {
        if (mConnected) {
            System.out
                    .println("获取MCU FW版本号:\n");
            if (commandqueryMCUFW == null) {
                commandqueryMCUFW = new HwtxCommandQueryMCUFW();
            }
            if (tagQueryMCUFW == null) {
                tagQueryMCUFW = commandqueryMCUFW.getCommandTagRaw();
            }

            sendData(commandqueryMCUFW.getBluetoothPropertyUuidSend(),
                    commandqueryMCUFW.getBluetoothPropertyUuidRead(),
                    commandqueryMCUFW.toBytes());
            System.out.println("1----------------------------");
        } else {
            EventBus.getDefault().post(
                    new ConnectBean(mConnected));
     ...
     * 获取BT FW版本号
     */
    private HwtxCommandQueryBTFW commandqueryBTFW = null;
    private byte[] tagQueryBTFW = null;

    public void queryBTFW() {
        if (mConnected) {
            System.out
                    .println("获取BT FW版本号:\n");
            if (commandqueryBTFW == null) {
                commandqueryBTFW = new HwtxCommandQueryBTFW();
            }
            if (tagQueryBTFW == null) {
                tagQueryBTFW = commandqueryBTFW.getCommandTagRaw();
            }

            sendData(commandqueryBTFW.getBluetoothPropertyUuidSend(),
                    commandqueryBTFW.getBluetoothPropertyUuidRead(),
                    commandqueryBTFW.toBytes());
            System.out.println("1----------------------------");
        } else {
            EventBus.getDefault().post(
                    new ConnectBean(mConnected));
     ...
     * 获取设备串号
     */
    private HwtxCommandQuerySNNUM commandquerySNNUM = null;
    private byte[] tagQuerySNNUM = null;

    public void querySNNUM() {
        if (mConnected) {
            System.out
                    .println("获取设备串号:\n");
            if (commandquerySNNUM == null) {
                commandquerySNNUM = new HwtxCommandQuerySNNUM();
            }
            if (tagQuerySNNUM == null) {
                tagQuerySNNUM = commandquerySNNUM.getCommandTagRaw();
            }

            sendData(commandquerySNNUM.getBluetoothPropertyUuidSend(),
                    commandquerySNNUM.getBluetoothPropertyUuidRead(),
                    commandquerySNNUM.toBytes());
            System.out.println("1----------------------------");
        } else {
            EventBus.getDefault().post(
                    new ConnectBean(mConnected));
...
//            }
    ...
    /**
     * 烧录设备 手动与扫描TODO 0xYYYYYYYY 00：入组失败，0xYYYYYYYY 01：入组成功------------------《修改队员信息》-------------------------------------------------------
     */
    private HwtxCommandTXB commandAddIn = null;
    ...
    public void addIn(String deviceSn) {
        ...
        if (mConnected) {
            ...
            if (commandAddIn == null) {
                commandAddIn = new HwtxCommandTXB();
            }
            if (tagAddIn == null) {
    ...
    private void getReceiveMember(byte[] returenByte) {
        ...
        for (int i = 0; i < indexMemberInfoArray.size(); i++) {
            ...
            if (TextUtils.equals(App.getInstance().getBleDuiYuanName(), strDeviceSN)) {//相等则是自己
    ...
    public void displayData(byte[] bs) {
        ...
        if (Arrays.equals(tagByte, tagQueryMode)) {// 查询设备号返回值
            byte[] mode = HwtxCommandUtility.extractBytesFromBytes(newByte, 4,
                    1);
            String utf8String = HwtxCommandUtility.bytesToHexLinet(mode);
            EventBus.getDefault().post(
                    new QueryLOGBean(3, "", "", utf8String, "", "", "", ""));

        } else if (Arrays.equals(tagByte, tagQueryMCUFW)) {// 获取MCU FW版本号
            byte[] mode = HwtxCommandUtility.extractBytesFromBytes(newByte, 9,
                    6);
            String utf8String = String.valueOf(HwtxCommandUtility.bytesToHexLinet(mode));
            EventBus.getDefault().post(
                    new QueryLOGBean(1, utf8String, "", "", "", "", "", ""));


        } else if (Arrays.equals(tagByte, tagQueryBTFW)) {// 获取BT FW版本号

            byte[] mode = HwtxCommandUtility.extractBytesFromBytes(newByte, 9,
                    6);
            String utf8String = HwtxCommandUtility.bytesToHexLinet(mode);
            EventBus.getDefault().post(
                    new QueryLOGBean(2, "", utf8String, "", "", "", "", ""));


        } else if (Arrays.equals(tagByte, tagQuerySNNUM)) {// 设备串号

            byte[] mode = HwtxCommandUtility.extractBytesFromBytes(newByte, 9,
                    6);
            String utf8String = HwtxCommandUtility.bytesToHexLinet(mode);
            EventBus.getDefault().post(
                    new QueryLOGBean(4, "", "", "", utf8String, "", "", ""));
            ...
            byte[] byt0 = {33};
            byte[] tap = HwtxCommandUtility.extractBytesFromBytes(newByte, 8,
                    1);
            if (Arrays.equals(tap, byt0)) {
                byte[] mode = HwtxCommandUtility.extractBytesFromBytes(newByte, 9,
                        1);
                EventBus.getDefault().post(
                        new AddInBean(mode));
            ...
            if (newByte.length == (15 + lengthInt)) {// 如果总长度等于准备传输的长度（15为固定值）+备参数表数据长度时执行
                ...
                String utf8String = HwtxCommandUtility.bytesToHexLinet(returenByte);
                EventBus.getDefault().post(
                        new QueryLOGBean(6, "", "", "", "", "", utf8String, ""));
            }else if (lengthInt == 0) {
                EventBus.getDefault().post(
                        new LengthZero(1));
            }
            ...
            try {
                utf8String = new String(
                        new String(bs, "GB2312").getBytes("UTF-8"));
            } catch (UnsupportedEncodingException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            EventBus.getDefault().post(
                    new QueryLOGBean(7, "", "", "", "", "", "", utf8String));
        }
    }
}
